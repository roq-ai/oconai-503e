...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.option_contract?.length > 0) {
      const create_option_contract = body.option_contract;
      body.option_contract = {
        create: create_option_contract,
      };
    } else {
      delete body.option_contract;
    }
